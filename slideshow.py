...
fontsize_intro = 20 # Размер шрифта для интро и аутро
...
def intro() :
    global text_logo, SIZE, fontsize_intro
    duration_intro = 2 # Длительность каждого текстового клипа
    logo1 = (TextClip(txt=text_logo,color="#0000AA", align='West',fontsize=fontsize_intro,font = 'Arial').set_duration(duration_intro).margin(right=8, top=8, opacity=0).set_pos(("center","center"))) # (optional) logo-border padding.set_pos(("right","top")))
...
def save_clip() :
    ...
    for name in file_names:
        ...
        clip = clip.resize(height=H).set_start(time_start).crossfadein(0.8)
    ...
    if os.path.isfile(file_name_text) :    
        ...
        with open(file_name_text, 'r', encoding='utf8') as fp:
            for line in fp:
                ...
                clip_text = (TextClip(txt=line.strip(),color="#0000AA",bg_color="rgba(255, 255, 255, 0.5)", align='West', fontsize=fontsize_intro, font = 'Arial')
    ...
    # Add logo
    if text_logo:
        logo = (TextClip(txt=text_logo, color='white', align='West',fontsize=16,font = 'Arial-Bold').set_duration(max_time).margin(right=8, top=8, opacity=0).set_pos(("right","top")))
